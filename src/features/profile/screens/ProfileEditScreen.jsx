...
import ViewerPlatformField from "@/features/profile/components/ViewerPlatformField";
import { ApiError, apiFetch, auth, uploads } from "@/shared/api/api-client";
import { uploadPublicUrl } from "@/shared/api/upload-url";
import { useCurrentUser } from "@/shared/auth/use-current-user";

const MAX_AVATAR_BYTES = 3 * 1024 * 1024;

function profileUserKey(user) {
  return user?.id || user?.userId || user?.accountId || "current";
}
...
export default function ProfileEditScreen() {
  ...
  const hydratedUserKey = useRef(null);
  ...
  useEffect(() => {
    ...
    if (user) {
      const userKey = profileUserKey(user);
      if (hydratedUserKey.current === userKey && dirtyRef.current) return;
      const next = profileValues(user);
      setDisplayName(next.displayName);
      setViewer({
        platform: next.viewerPlatform,
        nickname: next.viewerNickname,
      });
      setAvatarR2Key(next.avatarR2Key);
      hydratedUserKey.current = userKey;
    }
  }, [loading, user, router]);

  const original = useMemo(() => profileValues(user), [user]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if ("viewerPlatform" in patch || "viewerNickname" in patch) {
        setViewer({
          platform: "viewerPlatform" in patch ? patch.viewerPlatform : viewer.platform,
          nickname: "viewerNickname" in patch ? patch.viewerNickname || "" : viewer.nickname,
        });
      }
      if ("avatarR2Key" in patch) setAvatarR2Key(patch.avatarR2Key);
      await refresh();
  ...
  const avatarUrl = uploadPublicUrl(avatarR2Key);
