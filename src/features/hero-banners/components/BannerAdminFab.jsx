...
import { ApiError, heroBanners as bannersApi, uploads } from "@/shared/api/api-client";
import { uploadPublicUrl } from "@/shared/api/upload-url";
import { useCurrentUser, isAdmin } from "@/shared/auth/use-current-user";
...
function Drawer({ onClose, onChanged }) {
  ...
  const handleUploadSlot = async (idx, file) => {
    ...
    try {
      ...
      const url = res?.publicUrl || res?.url || res?.upload?.publicUrl || res?.upload?.url;
      const r2Key = res?.upload?.r2Key || res?.r2Key;
      const imageUrl = url || uploadPublicUrl(r2Key);
      if (!imageUrl) {
