...
import { ApiError, contests as contestsApi } from "@/shared/api/api-client";
...
export default function ContestForm({ contestId, schema = [], dnfProfile }) {
  ...
  useEffect(() => {
    ...
    setValues((prev) => {
      const prefilled = initialValues(schema, dnfProfile);
      const next = { ...prev };
      for (const [key, value] of Object.entries(prefilled)) {
        if (next[key] === "" || next[key] == null) next[key] = value;
      }
      return next;
    });
  }, [dnfProfile, schema]);
  ...
  async function handleFile(key, file) {
    if (!file) return;
    const seq = (uploadSeqRef.current[key] || 0) + 1;
    uploadSeqRef.current[key] = seq;
    ...
    try {
      ...
      if (uploadSeqRef.current[key] !== seq) return;
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (submittingRef.current || submitting || success) return;
