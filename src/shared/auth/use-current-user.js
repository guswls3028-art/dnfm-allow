...
import { ApiError, auth } from "@/shared/api/api-client";
...
function shouldTrySessionRefresh() {
  ...
  try {
    const last = Number(window.localStorage.getItem(REFRESH_CHECK_KEY) || 0);
    if (last && Date.now() - last < REFRESH_CHECK_TTL_MS) return false;
    window.localStorage.setItem(REFRESH_CHECK_KEY, String(Date.now()));
    return true;
  } catch {
    return true;
  }
...
function useCurrentUserValue() {
  ...
  const readCurrentUser = useCallback(async () => {
    try {
      const data = await auth.me({ skipAuthRefresh: true });
      const current = unwrapAuthUser(data);
      if (current) return current;
      if (shouldTrySessionRefresh()) {
  ...
  const login = useCallback(async ({ username, password, rememberMe = true }) => {
    const data = await auth.loginLocal({ username, password, rememberMe });
    const current = unwrapAuthUser(data);
    setUser(current);
    setError(null);
    setLoading(false);
  ...
  return useMemo(
    ...
    [error, fetchMe, loading, login, logout, user],
  );
}
