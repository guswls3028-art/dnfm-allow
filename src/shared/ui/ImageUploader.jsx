 ...
 * onChange 는 React setState-compatible updater 를 받는 형태로 사용한다.
...
export default function ImageUploader({ value = [], onChange, max = 5 }) {
    ...
    const toUpload = files.slice(0, remaining);
      ...
      if (newKeys.length > 0) {
        onChange((current = []) => {
          ...
          if (room <= 0) return currentKeys;
          return [...currentKeys, ...newKeys.slice(0, room)];
        });
  ...
  function remove(idx) {
    onChange((current = []) => {
      const currentKeys = Array.isArray(current) ? current : [];
      return currentKeys.filter((_, i) => i !== idx);
    });
  }
